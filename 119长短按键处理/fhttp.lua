...
local function cbFnc(result,prompt,head,body)
    ...
    if result and body then
        ...
        if result and type(tjsondata)=="table" then
            log.info("具体地址",tjsondata["regeocode"]["formatted_address"])
            loc=tjsondata["regeocode"]["formatted_address"]
            log.info("具体地址",loc)
...
function getLoc()
    ...
    A2=fgps.returnBlat()
    log.info("测试测试测试", A1)
    http.request("GET","https://restapi.amap.com/v3/geocode/regeo?output=json&&key=cca70f841558ef0ee3d3ffc6757325e1&location="..A1..","..A2.."&poitype=&radius=100&extensions=base&batch=false&roadlevel=0",nil,nil,nil,nil,cbFnc)
    return loc
end

function acqLocThread()
    while true do
        local result,data = sys.waitUntil("acq")
        if result then
            A1=fgps.returnBlng()
            A2=fgps.returnBlat()
            if data == "ttsLoc" then
...
-- http.request("GET","https://restapi.amap.com/v3/geocode/regeo?output=json&&key=cca70f841558ef0ee3d3ffc6757325e1&location=106.65404293703266,26.445573600596724&poitype=&radius=100&extensions=base&batch=false&roadlevel=0",nil,nil,nil,nil,cbFnc)
